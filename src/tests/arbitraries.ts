...
const words = (): Arbitrary<string> =>
  fc.array(word()).map((xs) => xs.join(' '))
...
const state = (): Arbitrary<types.State> =>
  fc.constantFrom(...locationPostalCodes.map(([, code]) => code))
...
const birthYear = (): Arbitrary<number> =>
  fc.integer({
    min: 1900,
    max: CURRENT_YEAR
  })
...
const account = (): Arbitrary<string> =>
  fc.integer({ min: 4, max: 17 }).chain((len) => numStr(len))

const email = (): Arbitrary<string> =>
  fc.tuple(word(), word(), word()).map(([w1, w2, w3]) => `${w1}@${w2}.${w3}`)

const phoneNumber = (): Arbitrary<string> => numStr(10)
...
export const f1099DivData = (): Arbitrary<types.F1099DivData> =>
  fc
    ...
    .map(([dividends, qualifiedDividends]) => ({
      dividends,
      qualifiedDividends
    }))
...
export const f1099Int = (): Arbitrary<types.Income1099Int> =>
  fc.tuple(payerName(), f1099IntData()).map(([payer, form]) => ({
...
export const f1099B = (): Arbitrary<types.Income1099B> =>
  fc.tuple(payerName(), f1099BData()).map(([payer, form]) => ({
...
export const f1099Div = (): Arbitrary<types.Income1099Div> =>
  fc.tuple(payerName(), f1099DivData()).map(([payer, form]) => ({
...
const f1098e = (): Arbitrary<types.F1098e> =>
  fc.tuple(maxWords(2), interest()).map(([lender, interest]) => ({
    lender,
    interest
  }))
...
export const person = (): Arbitrary<types.Person> =>
  fc.tuple(word(), word(), ein()).map(([firstName, lastName, ssid]) => ({
    firstName,
    lastName,
    ssid,
...
export const spouse = (): Arbitrary<types.Spouse> =>
  fc.tuple(person(), fc.boolean()).map(([person, isTaxpayerDependent]) => ({
    ...person,
    isTaxpayerDependent
  }))
