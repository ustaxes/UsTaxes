...
describe('f1040', () => {
  ...
  it('should never produce higher tax than total income', async () => {
    await testKit.with1040Assert((forms): Promise<void> => {
      ...
      expect(f1040).not.toBeUndefined()
      if (f1040 !== undefined) {
        // Remove l17 (AMT) and schedule 2 l13 (uncollected SS/Medicare on tips
        // from box12 codes A/B/M/N), as these pass-through employer-obligation
        // taxes can independently exceed taxable income for extreme inputs.
        expect(
          displayRound(
            f1040.l24() - (f1040.l17() ?? 0) - (f1040.schedule2.l13() ?? 0)
          ) ?? 0
